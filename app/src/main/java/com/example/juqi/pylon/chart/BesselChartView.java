...
import com.example.juqi.pylon.chart.BesselChart.ChartListener;
import com.example.juqi.pylon.chart.ChartData.Label;
...
class BesselChartView extends View {
    ...
    public BesselChartView(final Context context, final ChartData data, ChartStyle style, final BesselCalculator calculator) {
        super(context);
        ...
        this.detector = new GestureDetector(getContext(), new SimpleOnGestureListener() {
            ...
            public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
                scroller.fling((int) BesselChartView.this.calculator.getTranslateX(), 0, (int) velocityX, 0, -getWidth()*2, 0, 0, 0);
                ViewCompat.postInvalidateOnAnimation(BesselChartView.this);
                return true;
            }
            ...
            public boolean onSingleTapUp(MotionEvent e){
                float x = e.getX() - calculator.getTranslateX();
                float y = e.getY();
                Log.d("bessel",x+","+y);
                for (Series series : data.getSeriesList()) {
                    for (Point point : series.getPoints()) {
                        if (point.willDrawing) {
                            if(x>=(point.x-50)&&x<=(point.x+50)&&y>=(point.y-80)&&y<=(point.y+20)){
                                //Looper.prepare();
                                AlertDialog.Builder builder = new AlertDialog.Builder(context);
                                builder.setMessage(point.valueX + "#塔海拔为：" + point.valueY + " m");
                                builder.setTitle("提示");
    ...
    public void animateScrollToEnd(int duration) {
        ...
        for (Series series : data.getSeriesList()) {
            for (Point point : series.getPoints()) {
                if (point.willDrawing && point.isChosen) {
                    choseX = (int)point.x;
                }
            }
        }
        scroller.startScroll(0, 0,  -choseX + calculator.xAxisWidth / 6  , 0, duration);
    }
    ...
    private void drawGrid(Canvas canvas) {
        ...
        for (Point point : calculator.gridPoints) {
            if (point != null && point.willDrawing && point.valueY > 0)
                canvas.drawLine(point.x, point.y, point.x, calculator.yAxisDrawHeight, paint);
        }
    }
        ...
        for (Series series : data.getSeriesList()) {
            ...
            for (Point point : series.getPoints()) {
                if (point.willDrawing && !point.isChosen) {
                    ...
                    paint.setAlpha(255);
                    Bitmap bitmap = ((BitmapDrawable)getResources().getDrawable(R.drawable.tower)).getBitmap();
                    Bitmap towerBitmap = setBitmapSize(bitmap,60,60);
                    canvas.drawBitmap(towerBitmap,point.x-30,point.y- 60,paint);

                }else if(point.willDrawing && point.isChosen){
                    canvas.drawCircle(point.x, point.y, 5, paint);
                    paint.setAlpha(80);
                    canvas.drawCircle(point.x, point.y, 10, paint);
                    paint.setAlpha(255);
                    Bitmap bitmap = ((BitmapDrawable)getResources().getDrawable(R.drawable.tower_green)).getBitmap();
                    Bitmap towerBitmap = setBitmapSize(bitmap,100,100);
                    canvas.drawBitmap(towerBitmap,point.x-50,point.y- 100,paint);
                }
            }
            if (drawBesselPoint) {
    ...
    private void drawHorLabels(Canvas canvas) {
        ...
        float coordinateY =  data.getYLabels().get(data.getYLabels().size() - 1).y;
        canvas.drawLine(0, coordinateY, endCoordinateX, coordinateY, paint);
        for (Label label : data.getXLabels()) {
            // 绘制橫坐标文本
            canvas.drawText(label.text, label.x, label.y-30, paint);
        }
    }
