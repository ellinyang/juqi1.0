...
class BesselCalculator {
    ...
    public boolean ensureTranslation() {
        if (translateX >= 0) {
            ...
            if (yAxisWidth != 0 && translateX < -xAxisWidth / 3 * 2) {
                translateX = -xAxisWidth / 3  *2;
                return true;
            }
        }
        return false;
    ...
    private void computeVerticalAxisInfo() {
        ...
        yAxisDrawHeight = (int) data.getYLabels().get(data.getYLabels().size() - 1).y + xAxisHeight;
    }
    ...
    private void computeTitlesInfo() {
        ...
        for (Title title : titles) {
            ...
            title.circleX = title.textX - title.textRect.width() / 3 - title.circleTextPadding - title.radius;
